...
contract TokenizedBallot {
    ...
    function vote(uint proposal, uint256 amount) external {
        uint256 voteAllocation = tokenContract.getPastVotes(msg.sender, targetBlockNumber);
        uint256 votesSpentAfter = votingPowerSpent[msg.sender] + amount;
        require(votesSpentAfter <= voteAllocation, "Vote amount takes voter over allocation");
